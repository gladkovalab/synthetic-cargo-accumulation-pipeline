...
Based on CellProfiler pipeline 231115_combined_pipeline_new_nomito_fixed_moments
Pipeline Version: v6
"""
...
@dataclass
class ImagePair:
    """Represents a matched set of Hoechst, MIRO160mer, and (optional) 561 marker images."""
    ...
    marker561_path: Path | None = None

    @property
    def image_id(self) -> str:
        """Unique identifier for this image pair."""
        if self.timepoint:
            return f"{self.date}_{self.well_number}_{self.timepoint}_{self.sequence}_{self.xy_position}"
    ...
    def to_metadata_dict(self, image_number: int) -> dict[str, Any]:
        ...
        if self.timepoint:
            d["Metadata_Timepoint"] = self.timepoint
        if self.marker561_path is not None:
            d["FileName_Marker561"] = self.marker561_path.name
            d["PathName_Marker561"] = str(self.marker561_path.parent)
        return d
...
class FileDiscovery:
    ...
    def find_image_pairs(input_dir: Path, date: str | None = None) -> list[ImagePair]:
        ...
        for date_dir in date_dirs:
            ...
            # Separate by channel
            hoechst_files = [f for f in tif_files if "_405" in f.stem or "_20x DAPI" in f.stem]
            miro_files = [f for f in tif_files if "_488" in f.stem or "_20x GFP" in f.stem]
            marker561_files = [f for f in tif_files if f.stem.endswith("_561")]

            # Match pairs
            pairs = FileDiscovery._match_image_pairs(
                hoechst_files, miro_files, date_name, marker561_files=marker561_files
            )
    ...
    def _match_image_pairs(
        ...
        date: str,
        marker561_files: list[Path] | None = None,
    ) -> list[ImagePair]:
        """
        Match Hoechst, MIRO160mer, and (optional) 561 marker files by their base names.

        Args:
            hoechst_files: List of Hoechst (405nm) file paths
            miro_files: List of MIRO160mer (488nm) file paths
            date: Date identifier
            marker561_files: Optional list of 561nm marker file paths
        ...
        for h_file in hoechst_files:
            ...
            if parsed:
                key = (parsed["well"], parsed["sequence"], parsed["xy"], parsed.get("timepoint"))
                hoechst_dict[key] = (h_file, parsed)

        # Index 561 marker files by the same key
        marker561_dict: dict[tuple, Path] = {}
        for m_file in marker561_files or []:
            parsed = FileDiscovery._parse_filename(m_file.stem)
            if parsed:
                key = (parsed["well"], parsed["sequence"], parsed["xy"], parsed.get("timepoint"))
                marker561_dict[key] = m_file
            ...
            if parsed:
                ...
                if key in hoechst_dict:
                    ...
                    pair = ImagePair(
                        ...
                        marker561_path=marker561_dict.get(key),
                    )
                    pairs.append(pair)
    ...
    def _parse_filename(filename: str) -> dict[str, str] | None:
        ...
        Supported formats:
            1. Standard XY format: [Plate000_]Well{WELL}_Channel..._Seq{SEQ}[-MaxIP]_XY{N}_{CHANNEL}
               Example: "WellD10_Channel405,561,488,640_Seq0009-MaxIP_XY9_405"
               Example: "WellB02_Channel20x GFP,20x DAPI_Seq0000-MaxIP_XY1_20x DAPI"

            2. crop_T1/t1_T1 format with XY: Well{WELL}_Channel..._Seq{SEQ}-MaxIP_{PREFIX}_XY{N}_{CHANNEL}
               Example: "WellE03_Channel405,561,488,640_Seq0003-MaxIP_crop_T1_XY8_405"

            3. 4-digit format: [Plate000_]Well{WELL}_Channel..._Seq{SEQ}-MaxIP_{WELL}_{0000-9999}_{CHANNEL}
               Example: "WellF02_Channel405,561,488,640_Seq0001-MaxIP_F02_0007_405"
               Example: "Plate000_WellB02_Channel405,561,488,640_Seq0000-MaxIP_B02_0000_405"
        ...
        pattern_xy = r"^(?:Plate\d+_)?Well([A-Z]\d+)_Channel.+?_Seq(\d+)(?:-MaxIP)?_XY(\d+)_(.+)$"
        ...
        # Pattern 2: crop_T1 or t{N} prefix format with XY
        # WellXXX_ChannelYYY_SeqZZZZ-MaxIP_{crop_T1|t{N}[_T{N}]}_XY{N}_{CHANNEL}
        pattern_crop_t1 = (
            r"^Well([A-Z]\d+)_Channel.+?_Seq(\d+)-MaxIP_(?:crop_T1|(t\d+)(?:_T\d+)?)_XY(\d+)_(.+)$"
        )
        ...
        if match:
            return {
                "well": match.group(1),
                "sequence": f"Seq{match.group(2)}",
                "timepoint": match.group(3),  # None for crop_T1, "t1"/"t2"/etc. for t prefix
                "xy": f"XY{match.group(4)}",
                "channel": match.group(5),
            }

        # Pattern 3: 4-digit position format (without crop_T1/t1_T1)
        # [Plate000_]WellXXX_ChannelYYY_SeqZZZZ-MaxIP_{WELL}_{0000-9999}_{CHANNEL}
        # Position is 0-indexed, XY is 1-indexed: 0000 -> XY1, 0007 -> XY8
        pattern_4digit = (
            r"^(?:Plate\d+_)?Well([A-Z]\d+)_Channel.+?_Seq(\d+)-MaxIP_[A-Z]\d+_(\d{4})_(.+)$"
        )
        ...
        # Pattern 4: 4-digit position format WITH crop_T1 or t{N} prefix
        # [Plate000_]WellXXX_ChannelYYY_SeqZZZZ-MaxIP_{crop_T1|t{N}[_T{N}]}_{WELL}_{0000-9999}_{CHANNEL}
        # Example: WellB02_Channel405,561,488,640_Seq0000-MaxIP_crop_T1_B02_0000_405
        # Example: WellB02_Channel405,561,488,640_Seq0000-MaxIP_t1_T1_B02_0000_405
        # Position is 0-indexed, XY is 1-indexed: 0000 -> XY1, 0007 -> XY8
        pattern_4digit_crop = r"^(?:Plate\d+_)?Well([A-Z]\d+)_Channel.+?_Seq(\d+)-MaxIP_(?:crop_T1|(t\d+)(?:_T\d+)?)_[A-Z]\d+_(\d{4})_(.+)$"
...
class ImageLoader:
    ...
    def load_image(file_path: Path) -> np.ndarray:
        ...
        if image.ndim > 2:
            logger.warning(
                f"Image has {image.ndim} dimensions {image.shape}, "
                "reducing to 2D (max projection or first slice)"
            )
            # shape[0] < shape[-1] suggests channels-first (e.g., 3x1024x1024) -> max project
            # otherwise assume first axis is spatial -> take first slice
    ...
    @staticmethod
    def load_marker561(image_pair: ImagePair) -> np.ndarray | None:
        """
        Load the 561 marker image if present, otherwise return None.

        Args:
            image_pair: ImagePair object with optional marker561_path

        Returns:
            2D float array of 561 marker, or None if not available
        """
        if image_pair.marker561_path is None:
            return None
        return ImageLoader.load_image(image_pair.marker561_path)
...
class CSVExporter:
    ...
    def _create_all_measurements(
        ...
        for _, img_row in image_df.iterrows():
            ...
            # Count nuclei for this image (interior only, from Nuclei.csv)
            ...
            # Get nuclei counts (all vs interior) from Image measurements
...
def _parse_well_timepoint(well_str: str) -> tuple[str, str | None]:
    """
    Parse a well string that may include a timepoint suffix.
    ...
    Args:
        well_str: Well string, optionally with timepoint suffix

    Returns:
        Tuple of (well, timepoint_or_None)
    """
    parts = well_str.strip().split()
    if len(parts) == 2 and re.match(r"^t\d+$", parts[1]):
        return parts[0], parts[1]
    return well_str.strip(), None


def detect_figure_sheets(
    excel: pd.ExcelFile, restrict_to: list[str] | None = None
) -> list[str]:
    """Pick figure sheets out of an Excel file, optionally restricted to a subset.

    Matches the rule used by figure_aggregation:
    - prefer sheets whose name starts with "Fig"
    - fall back to "all sheets except Otsu_params/Aggregate/Figures"

    If ``restrict_to`` is given, return only sheets that are both in the workbook
    and in that list (preserves the workbook order). Raises if any name in
    ``restrict_to`` doesn't exist in the workbook.
    """
    ...
    if restrict_to is not None:
        missing = [s for s in restrict_to if s not in excel.sheet_names]
        if missing:
            raise ValueError(
                f"Figure sheet(s) not found in {excel.io}: {missing}. "
                f"Available: {excel.sheet_names}"
            )
        wanted = set(restrict_to)
        fig_sheets = [s for s in fig_sheets if s in wanted]
    return fig_sheets


def extract_figure_inclusions(
    config_path: Path, figure_sheets: list[str] | None = None
) -> set[tuple[str, str, str | None]]:
    """
    Extract all (date, well, timepoint) triples from Figure sheets in config Excel file.

    This parses all Figure sheets and extracts every date/well/timepoint combination,
    which can be used to filter images to only process those needed for figures.

    Well values may include timepoint suffixes (e.g., "B03 t1").

    Args:
        config_path: Path to Excel config file with Figure sheets
        figure_sheets: Optional list of sheet names to restrict to. None = all
            figure sheets (current behavior).

    Returns:
        Set of (date, well, timepoint) tuples, e.g.,
        {('230912', 'B02', None), ('221216', 'B03', 't1')}
    """
    config_path = Path(config_path)
    excel = pd.ExcelFile(config_path)
    fig_sheets = detect_figure_sheets(excel, restrict_to=figure_sheets)

    inclusions: set[tuple[str, str, str | None]] = set()
        ...
        try:
            ...
            for _, row in fig_df.iterrows():
                ...
                for condition in conditions:
                    ...
                    # Handle comma-separated wells (e.g., "B06,D06,B07" or "B03 t1")
                    wells = [w.strip() for w in str(well_val).split(",")]
                    for well in wells:
                        if well:
                            well_id, timepoint = _parse_well_timepoint(well)
                            inclusions.add((date_str, well_id, timepoint))

        except Exception as e:
            logger.warning(f"Error parsing figure sheet {sheet_name}: {e}")

    logger.info(
        f"Extracted {len(inclusions)} (date, well, timepoint) triples "
        f"from {len(fig_sheets)} figure sheets"
    )
    return inclusions


def should_include_image(
    image_pair: ImagePair, inclusions: set[tuple[str, str, str | None]] | None
) -> bool:
    """
    ...
    Args:
        image_pair: ImagePair to check
        inclusions: Set of (date, well, timepoint) tuples to include, or None to include all
    ...
    # Exact match including timepoint
    if (image_pair.date, image_pair.well_number, image_pair.timepoint) in inclusions:
        return True
    # Match entries without timepoint constraint (None means "any timepoint")
    if (image_pair.date, image_pair.well_number, None) in inclusions:
        return True
    return False
