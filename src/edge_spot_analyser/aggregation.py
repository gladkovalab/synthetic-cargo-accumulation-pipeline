...
class Aggregator:
    ...
    def _process_edge_spots(self, input_path: Path, output_dir: Path) -> None:
        ...
        has_timepoints = processed_df["T"].nunique() > 1

        if self.config.t_varies or has_timepoints:
        ...
        if not has_timepoints:
            ...
            # Generate per-timepoint static files
            for t_val in sorted(processed_df["T"].unique()):
                t_df = processed_df[processed_df["T"] == t_val]
    ...
    def _extract_edgespot_cols(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Always extract T to auto-detect timepoints (even without --t-varies)
    ...
    def _generate_edge_spot_static_files(
        self, df: pd.DataFrame, output_dir: Path, timepoint: str | None = None
    ) -> None:
        ...
        for col_name, file_stem in burden_metrics:
            if col_name in df.columns:
                output_path = output_dir / f"{file_stem}{suffix}.csv"
                self._save_pivot_table(
                    data=df,
                    values=col_name,
                    index="XY",
    ...
    def _process_bonus_cols(self, input_path: Path, output_dir: Path) -> None:
        ...
        for bonus_col in self.config.bonus_cols:
            if bonus_col in raw_df.columns:
                cols = ["WellNumber", "XY", "T", bonus_col]
    ...
    def _generate_ragged_df(
        ...
        has_timepoints = "T" in input_df.columns and input_df["T"].nunique() > 1
        ...
        if not has_timepoints:
            ...
            for t_val in sorted(input_df["T"].unique()):
                t_df = input_df[input_df["T"] == t_val]
                self._generate_ragged_static_files(
                    t_df, data_column, output_dir, timepoint=f"t{t_val}"
                )
    ...
    def _generate_ragged_static_files(
        ...
        output_path = output_dir / f"{data_column}_static{suffix}.csv"
        ...
        median_path = output_dir / f"{data_column}_fov_median_static{suffix}.csv"
        ...
        stacked_path = output_dir / f"{data_column}_stacked_static{suffix}.csv"
        logger.info(f"Writing stacked {data_column} table with shape {stacked_df.shape}")
        stacked_df.to_csv(stacked_path)
...
def extract_xy(input_string: str) -> int:
    ...
    match = re.search(r"(?<=_XY)\d+", input_string)
    if match:
        return int(match.group(0))
    # Fallback: 4-digit position after well ID (e.g., _B02_0007_)
    match = re.search(r"_[A-H]\d{2}_(\d{4})_", input_string)
    if match:
        return int(match.group(1))
    raise ValueError(f"Could not extract XY position from: {input_string}")


def extract_timestamp(input_string: str) -> int:
    """Extract experimental timepoint (lowercase t) from filename.

    Looks for _t{N}_ patterns (e.g., _t1_T1_, _t2_T1_).
    The lowercase t is the experimental timepoint; the uppercase T is a
    CellProfiler artifact and always 1 for max-IP projections.
    """
