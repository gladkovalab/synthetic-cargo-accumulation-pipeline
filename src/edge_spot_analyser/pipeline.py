...
def _process_single_image(args: tuple) -> dict[str, Any]:
    ...
    try:
        ...
        marker561 = ImageLoader.load_marker561(image_pair)
...
class Pipeline:
    ...
    def __init__(
        ...
        self.nuclei_params = (
            nuclei_params if nuclei_params is not None else NucleiSegmentationParams()
        )
        self.edge_spot_params = (
            edge_spot_params if edge_spot_params is not None else EdgeSpotParams()
        )
        self.perinuclear_params = (
            perinuclear_params if perinuclear_params is not None else PerinuclearRegionParams()
        )
        self._current_exclusions: set[tuple[str, int]] = set()  # (well, xy) pairs to exclude
        self._figure_inclusions: set[tuple[str, str, str | None]] | None = None
    ...
    def load_figure_inclusions(
        self, config_path: Path, figure_sheets: list[str] | None = None
    ) -> None:
        """
        ...
        """
        self._figure_inclusions = extract_figure_inclusions(
            config_path, figure_sheets=figure_sheets
        )
...
Examples:
  ...
  edge-spot-pipeline --input inputs/ --output results/ --verbose
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Restrict --figures-only to a subset of Figure sheets "
            "(e.g. --figure-sheets FigS8A FigS8B). Default: all figure sheets in --config."
        ),
    )
    ...
    if args.figures_only:
        if not args.config:
            ...
            return
        pipeline.load_figure_inclusions(args.config, figure_sheets=args.figure_sheets)
    elif args.figure_sheets:
        logger.warning("--figure-sheets has no effect without --figures-only; ignoring")
    ...
    if args.date:
        ...
        if config:
            ...
            pipeline.update_parameters(date_params)
