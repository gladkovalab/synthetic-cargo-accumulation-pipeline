...
from edge_spot_analyser.io_utils import _parse_well_timepoint, detect_figure_sheets

logger = logging.getLogger(__name__)

_FILENAME_WELL_RE = re.compile(r"^(?:Plate\d+_)?Well([A-Z]\d+)_")


def _well_from_filename(filename: str) -> str | None:
    m = _FILENAME_WELL_RE.match(str(filename))
...
class FigureAggregator:
    ...
    def __init__(
        ...
        figure_sheets: list[str] | None = None,
    ):
        """
        ...
        Args:
            ...
            figure_sheets: Optional list of sheet names to restrict to
        """
        ...
        self.figure_sheets = detect_figure_sheets(self.excel, restrict_to=figure_sheets)
        logger.info(f"Found {len(self.figure_sheets)} figure sheets: {self.figure_sheets}")

        # Cache: date -> {well: count} of cells in Perinuclear_region.csv
        self._cells_per_well_by_date: dict[str, dict[str, int]] = {}

    def _cells_per_well_for_date(self, date_str: str) -> dict[str, int]:
        """Return {well: cell_count} from Perinuclear_region.csv for a date.

        Used to emit per-figure cell-count tables. Returns an empty dict if
        the file is missing.
        """
        if date_str in self._cells_per_well_by_date:
            return self._cells_per_well_by_date[date_str]
        peri_path = self.results_dir / date_str / "Perinuclear_region.csv"
        counts: dict[str, int] = {}
        if peri_path.exists():
            peri = pd.read_csv(peri_path, usecols=["FileName_Hoechst"])
            wells = peri["FileName_Hoechst"].map(_well_from_filename).dropna()
            counts = wells.value_counts().to_dict()
        self._cells_per_well_by_date[date_str] = counts
        return counts
    ...
    def process_figure(self, sheet_name: str) -> None:
        ...
        for _, row in fig_df.iterrows():
            ...
            counts_for_date = self._cells_per_well_for_date(date_str)
            ...
            for condition in conditions:
                ...
                for well_str in wells:
                    well, timepoint = _parse_well_timepoint(well_str)
                    col_name = f"{self._sanitize_name(condition)}_{date_str}_{well}"
                    if timepoint:
                        col_name += f"_{timepoint}"
                    ...
                    cell_count_rows.append(
                        {
                            ...
                            "cells": int(counts_for_date.get(well, 0)),
                        }
                    )

                    # Try to load each metric
                    for metric_name, filename in ALL_METRICS:
                        col_data = self._load_well_data(
                            date_str, well, filename, timepoint=timepoint
                        )
        ...
        # Write per-figure cell-count breakdown (used by edge-spot-plot to
        # annotate gini plots with N per condition)
        if cell_count_rows:
            counts_df = pd.DataFrame(cell_count_rows)
            counts_path = self.output_dir / f"{sheet_name}_cell_counts.csv"
            counts_df.to_csv(counts_path, index=False)
            logger.info(f"Wrote cell counts to {counts_path}")

    def _load_well_data(
        self, date_str: str, well: str, filename: str, timepoint: str | None = None
    ) -> pd.Series | None:
        """
        ...
        Args:
            ...
            timepoint: Optional timepoint (e.g., 't1') - looks for timepoint-suffixed file

        Returns:
            Series with XY as index, or None if not found
        """
        # If timepoint specified, try the per-timepoint file first
        if timepoint:
            stem, ext = filename.rsplit(".", 1)
            tp_filename = f"{stem}_{timepoint}.{ext}"
            tp_path = self.results_dir / date_str / tp_filename
            if tp_path.exists():
                return self._read_well_from_csv(tp_path, well)
        ...
        return self._read_well_from_csv(file_path, well)

    def _read_well_from_csv(self, file_path: Path, well: str) -> pd.Series | None:
        """Read a single well's data from a CSV file."""
        try:
            ...
            if "XY" in df.columns:
                df = df.set_index("XY")
                # Keep only well columns matching plate format (e.g., B02, D02, E02)
                well_cols = [c for c in df.columns if re.match(r"^[A-H]\d{2}$", str(c))]
                if well_cols:
                    df = df[well_cols]
            else:
...
Examples:
  ...
  edge-spot-figure-aggregate -r results/ -c config.xlsx -o figures/
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Restrict to a subset of figure sheets (default: all figure sheets in --config)",
    )
