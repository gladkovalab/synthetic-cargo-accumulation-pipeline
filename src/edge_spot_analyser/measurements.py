...
def combine_measurements_for_export(
    ...
    marker561_image: np.ndarray | None = None,
    ...
    # Add per-cell 561 marker intensities to the perinuclear ring (used for
    # filtering out cells that don't express the cargo marker).
    if marker561_image is not None and not perinuclear_df.empty:
        perinuclear_marker561_df = measure_all_object_properties(
            perinuclear_region_labels,
            marker561_image,
            channel_name="Marker561",
            include_edge_intensity=False,
            include_mass_displacement=False,
            include_gini=False,
            include_moments=False,
        )
        # Drop the duplicated `area` column before merging on ObjectNumber
        perinuclear_marker561_df = perinuclear_marker561_df.drop(
            columns=[c for c in perinuclear_marker561_df.columns if c == "area"]
        )
        perinuclear_df = perinuclear_df.merge(
            perinuclear_marker561_df, on="ObjectNumber", how="left"
        )
