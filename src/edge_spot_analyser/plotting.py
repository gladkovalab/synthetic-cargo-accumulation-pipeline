...
def create_figure(
    ...
    cell_counts_per_condition: dict[str, int] | None = None,
):
    """
    ...
    for i, (_condition, cond_data) in enumerate(data.items()):
        ...
        if individual:
            ...
            y_max_seen = max(y_max_seen, float(np.nanmax(individual)))
        ...
        if replicate_means:
            ...
            y_max_seen = max(y_max_seen, float(np.nanmax(replicate_means)))

    # Optional per-condition N annotation (e.g. number of cells contributing)
    if cell_counts_per_condition and np.isfinite(y_max_seen):
        annot_y = y_max_seen * 1.04
        for i, cond in enumerate(conditions):
            n = cell_counts_per_condition.get(cond)
            if n is None:
                continue
            ax.text(
                x_positions[i],
        ...
        cur_lo, cur_hi = ax.get_ylim() if ax.has_data() else (0.0, annot_y)
        ax.set_ylim(cur_lo, max(cur_hi, annot_y * 1.1))
...
def _load_cell_counts_per_condition(
    csv_path: Path, fig_name: str, metric: str
) -> dict[str, int] | None:
    """Look up <Figure>_cell_counts.csv next to a metric CSV.

    Returns {sanitized_condition: total_cells} or None if absent. Only used
    to annotate the gini plot — every other metric is unaffected.
    """
    if "gini" not in metric:
        return None
    counts_path = csv_path.parent / f"{fig_name}_cell_counts.csv"
    ...
    if counts_df.empty:
        return None
    counts_df["_cond"] = (
        counts_df["condition"].astype(str).str.replace(" ", "_").str.replace("-", "_")
    )
    return counts_df.groupby("_cond")["cells"].sum().astype(int).to_dict()
...
def process_figure_file(
    ...
    # Annotate gini plots with per-condition cell counts when available
    cell_counts = _load_cell_counts_per_condition(csv_path, fig_name, metric)

    # Create and save figure (PNG + PDF from same figure to ensure identical jitter)
    fig = create_figure(data, title=title, ylabel=ylabel, cell_counts_per_condition=cell_counts)
